  ...
  render(){
    ...
    return(
      <tr onMouseEnter={this._handleMouseOver} onMouseLeave={this._handleMouseOut}>
        ...
        { 
          actions.length > 0 &&
          <td className='data-table-action-wrap'>{
            actions.map((action, index)=>(

            <span key={`action-${index}`}
                  className='data-table-action-button-wrap'
                  onClick={action.onClick && action.onClick.bind(null, rowIndex)}>
              
              <span className={`data-table-action-button ${action.label? 'with-text-label': 'icon-label-only'}`}>
